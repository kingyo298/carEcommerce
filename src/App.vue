<template>
  <div class="container mt-5">
    <div class="d-flex">
      <div class="mx-2">
        <select class="form-select" aria-label="Default select example" v-model="selectedCategory">
          ...
          <option v-for="(category,index) in getCategories" :key="index" :value="index">{{category}}</option>
        </select>
      </div>
      <div>
        <select class="form-select" aria-label="Default select example" v-model="selectedSort">
    ...
    <div class="d-flex flex-wrap pt-4">
      <app-item-card v-for="(car,index) in matchedCar" :key="index" :car="car"></app-item-card>
    </div>
  </div>
</template>
    ...
    computed : {
      getCategories(){
        ...
        for(var i = 0; i < this.cars.length; i++){
          if(categories.indexOf(this.cars[i].category) === -1){
            categories.push(this.cars[i].category);
          }
        }
        return categories;
      },
      ...
      matchedCar(){
        if(this.selectedCategory === "-1"){
          this.result = this.cars.slice(0,this.cars.length);
        }else{
          this.result = this.cars.filter((element) => {
            return element.category === this.getCategories[this.selectedCategory];
          },this);
        }
        this.sortCars;
        return this.result;
